...
class DishesRoutesTest {
    ...
    inner class GetAllDishes {
        @Test
        fun `returns empty list when no dishes exist`() = testApplication {
            setupDishesRoutes()
        ...
        @Test
        fun `returns all non-deleted dishes`() = testApplication {
            setupDishesRoutes()
        ...
        @Test
        fun `does not return deleted dishes`() = testApplication {
            setupDishesRoutes()
    ...
    inner class GetDishByUuid {
        @Test
        fun `returns dish when it exists`() = testApplication {
            setupDishesRoutes()
    ...
    inner class CreateDish {
        @Test
        fun `creates new dish with minimal properties`() = testApplication {
            setupDishesRoutes()
            val client = jsonClient()

            val response = client.post("/dishes") {
        ...
        @Test
        fun `creates vegetarian dish`() = testApplication {
            setupDishesRoutes()
            val client = jsonClient()
        ...
        @Test
        fun `creates dish with all dietary flags`() = testApplication {
            setupDishesRoutes()
            val client = jsonClient()
    ...
    inner class UpdateDish {
        @Test
        fun `updates existing dish`() = testApplication {
            setupDishesRoutes()
            val client = jsonClient()
            val dish = aDish(name = "Salad", description = "Fresh salad", isVegetarian = true)
            dishesService.insert(dish)

            val response = client.put("/dishes/${dish.uuid}") {
                contentType(ContentType.Application.Json)
                setBody(mapOf(
                    "name" to "Caesar Salad",
    ...
    inner class DeleteDish {
        @Test
        fun `soft deletes dish`() = testApplication {
            setupDishesRoutes()
            val dish = aDish(name = "Pasta", description = "Italian pasta")
            dishesService.insert(dish)
